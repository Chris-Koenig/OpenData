...
const convertLV95ToWGS84 = (east: number, north: number): { lat: number; lng: number } => {
  ...
  const lat = 46.95240 + 4.728982 * x - 0.791484 * y * y + 0.1306 * y * y * x - 0.0436 * x * x * x;
      ...
      Papa.parse(csvText, {
        ...
        complete: (results: any) => {
          ...
          for (let i = 0; i < maxRows; i++) {
            ...
            const eastStr = row['AccidentLocation_CHLV95_E'] || row.AccidentLocation_CHLV95_E;
            const northStr = row['AccidentLocation_CHLV95_N'] || row.AccidentLocation_CHLV95_N;
