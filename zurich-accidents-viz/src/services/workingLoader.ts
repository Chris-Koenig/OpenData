...
export async function loadAccidentDataWorking(): Promise<CoordinatePoint[]> {
  ...
  try {
    ...
    console.log(
      `📈 WORKING LOADER: Parse complete. Rows: ${parseResult.data.length}, Errors: ${parseResult.errors.length}`
    );
    ...
    console.log(
      `⚡ WORKING LOADER: Processing first ${batchSize} of ${totalRows} rows...`
    );
    ...
    console.log(
      `🎉 WORKING LOADER: Complete! Processed ${processedCount} valid points, skipped ${skippedCount} invalid rows`
    );
    console.log(
      `📊 WORKING LOADER: Sample points:`,
      points
        .slice(0, 3)
        .map((p) => ({
          lat: p.lat,
          lng: p.lng,
          type: p.accident.AccidentType_en,
        }))
    );
