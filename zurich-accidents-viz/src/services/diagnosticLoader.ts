...
export async function loadAccidentDataDiagnostic(): Promise<CoordinatePoint[]> {
  ...
  try {
    ...
    console.log(
      "📥 DIAGNOSTIC: Response headers: Content-Type =",
    ...
    console.log(
      `📍 DIAGNOSTIC: Looking for coordinate keys: "${eastKey}", "${northKey}"`
    );
    ...
    console.log(
      `📍 DIAGNOSTIC: Parsed coordinates: E=${eastVal}, N=${northVal}`
    );
    console.log(
      `📍 DIAGNOSTIC: Are valid numbers: E=${!isNaN(eastVal)}, N=${!isNaN(
        northVal
      )}`
    );
    ...
    if (!isNaN(eastVal) && !isNaN(northVal)) {
      ...
      console.log(
        `🌐 DIAGNOSTIC: Converted coordinates: lat=${converted.lat}, lng=${converted.lng}`
      );
      ...
      const testAccident: AccidentData = {
        ...
        AccidentSeverityCategory:
          firstRow["AccidentSeverityCategory"] || "unknown",
        AccidentSeverityCategory_en:
          firstRow["AccidentSeverityCategory_en"] || "Unknown",
        AccidentInvolvingPedestrian:
          firstRow["AccidentInvolvingPedestrian"] || "false",
        AccidentInvolvingBicycle:
          firstRow["AccidentInvolvingBicycle"] || "false",
        AccidentInvolvingMotorcycle:
          firstRow["AccidentInvolvingMotorcycle"] || "false",
