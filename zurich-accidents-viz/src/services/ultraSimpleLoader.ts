...
export async function loadAccidentDataUltraSimple(): Promise<CoordinatePoint[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('❌ Ultra Simple Loader: Fetch failed:', response.status, response.statusText);
    ...
    console.log(`📊 Ultra Simple Loader: CSV size: ${csvText.length} characters`);
    
    // Step 3: Remove BOM if present
    const cleanText = csvText.replace(/^\uFEFF/, '');
    console.log(`🧹 Ultra Simple Loader: After BOM removal: ${cleanText.length} characters`);
    ...
    console.log(`📈 Ultra Simple Loader: Parse complete. Rows: ${parseResult.data.length}, Errors: ${parseResult.errors.length}`);
    
    if (parseResult.errors.length > 0) {
      console.warn('⚠️  Ultra Simple Loader: Parse errors:', parseResult.errors.slice(0, 3));
    }
    ...
    console.log('🔍 Ultra Simple Loader: First row keys:', Object.keys(firstRow).slice(0, 10));
    console.log('🔍 Ultra Simple Loader: East coordinate:', firstRow['AccidentLocation_CHLV95_E']);
    console.log('🔍 Ultra Simple Loader: North coordinate:', firstRow['AccidentLocation_CHLV95_N']);
    ...
    for (let i = 0; i < testRows.length; i++) {
      ...
      if (isNaN(east) || isNaN(north)) {
        console.warn(`⚠️  Row ${i}: Invalid coordinates - E=${east}, N=${north}`);
        continue;
      }
      ...
      if (lat < 45.8 || lat > 47.9 || lng < 5.9 || lng > 10.5) {
        console.warn(`⚠️  Row ${i}: Coordinates outside Switzerland - lat=${lat}, lng=${lng}`);
        continue;
      }
      ...
      const accidentData: AccidentData = {
        ...
        AccidentSeverityCategory_en: row['AccidentSeverityCategory_en'] || 'Unknown',
    ...
    console.log(`🎉 Ultra Simple Loader: Complete! Generated ${points.length} points from ${testRows.length} test rows`);
