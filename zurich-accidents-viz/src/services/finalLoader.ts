...
export async function loadAccidentDataFinal(): Promise<CoordinatePoint[]> {
  ...
  try {
    ...
    console.log(
      `📊 FINAL LOADER: CSV loaded successfully (${csvText.length} characters)`
    );
    ...
    console.log(
      `⚡ FINAL LOADER: Processing ${processRows} of ${totalRows} rows...`
    );
      ...
      try {
        ...
        const accidentData: AccidentData = {
          ...
          AccidentSeverityCategory:
            row["AccidentSeverityCategory"] || "unknown",
          AccidentSeverityCategory_en:
            row["AccidentSeverityCategory_en"] || "Unknown Severity",
    ...
    console.log(
      `📊 FINAL LOADER: Successfully processed ${processedCount} valid points`
    );
    console.log(`⚠️  FINAL LOADER: Skipped ${skippedCount} invalid rows`);

    // Count accident types for summary
    const pedestrianCount = points.filter(
      (p) => p.accident.AccidentInvolvingPedestrian === "true"
    ).length;
    const bicycleCount = points.filter(
      (p) => p.accident.AccidentInvolvingBicycle === "true"
    ).length;
    const otherCount = points.length - pedestrianCount - bicycleCount;

    console.log(
      `📊 FINAL LOADER: Accident types - Pedestrian: ${pedestrianCount}, Bicycle: ${bicycleCount}, Other: ${otherCount}`
    );
